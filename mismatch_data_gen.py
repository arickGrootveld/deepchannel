...
#       arCoeffs (tuple) {default=(0.5,-04)} - the ARCoeffs that will be used for the F Matrix of the AR Process
#   Outputs: (data, params)
...
def ARDatagenMismatch(params, seed=int(torch.abs(torch.floor(100*torch.randn(1)))), cuda=False, arCoeffs=(0.5, -0.4), **kwards):
    ...
    for i in range(0,simLength):
        ...
        riccatiPred = sciAlg.solve_discrete_are(torch.t(F).numpy(),
                                         torch.t(H).numpy(),Q.numpy(), R.numpy())
