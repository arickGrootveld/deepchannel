...
This paper proposes a TCN-based (temporal convolutional network) structure for channel tracking and prediction and compares channel tracking performance of the Kalman filter and TCN-based channel tracking approach in three settings: (i) a simulated setting where the Kalman filter is known to be optimal, (ii) a simulated setting where the ideal assumptions required for Kalman filter optimality are broken and (iii) a simulated setting where the channel follows the Gilbert-Elliott model.  First, under the assumption that the channel obeys a simple linear, autoregressive (AR) model with additive white Gaussian noise, the performance of the proposed TCN approach is compared with a classical Kalman filter, and the performance gap in using the necessarily suboptimal TCN approach is quantified.  Next, the idealized assumptions of the channel model are relaxed so that there is mismatch between the presumed linear time-varying AR model and the true model, and results are again compared. Finally, a Gilbert-Elliott channel model is assumed where the channel oscillates between good and bad states and tracking performance is again evaluated. Results from using the least squares approach in all three scenarios as well as the Riccati equation solution are also compared.
...
Channel tracking and prediction are critical tasks in communication systems with mobility. The ability to accurately track and predict channels is necessary to enable many aspects of modern communication systems including coherent demodulation, rate and power control, and beamforming \cite{181202986}. Under an ideal AR channel model where, for example, the dynamics of the time-varying channel are linear, the noise is Gaussian, and the noise covariance matrices are known, the classical Kalman filter is known to be the optimal estimator and predictor.  When these idealized assumptions break down, however, or when the underlying dynamical model does not match the dynamics of the actual channel, the Kalman filter is no longer optimal and channel estimation performance can suffer significantly.

This paper applies very recent advances in machine learning for so-called "sequence prediction" problems to the task of channel tracking and prediction. While recurrent neural networks (RNNs) and long short-term memory (LSTM) have historically been the most popular choice for sequence prediction -- and, indeed, prior work has demonstrated promising results for their use in channel tracking \cite{7508408} \cite{hochreiter1997long} -- recent results in machine learning have shown that a new architecture called temporal convolution networks (TCNs) can often outperform LSTM-based RNNs in sequence prediction problems \cite{BaiTCN2018}.  In addition to their performance advantage in a wide range of sequence prediction problems, TCNs have a number of implementation advantages over RNNs in that they admit a more parallelizable implementation and have reduced memory requirement for training; in addition, TCNs have several structural advantages including an adjustable receptive field size and avoidance of the problem of exploding/vanishing gradients known to plague RNNs.
...
x&=&\begin{bmatrix}
   ...
   x[k-M]
\end{bmatrix}\\
\hat{x_{pred}}&=&\begin{bmatrix}
   \hat{x}[k-1]   \\
   \hat{x}[k-2] \\
   \vdots \\
   \hat{x}[k-M-1]
\end{bmatrix}\\
\hat{x_{est}}&=&\begin{bmatrix}
   \hat{x}[k]   \\
   \hat{x}[k-1] \\
   \vdots \\
   \hat{x}[k-M]
