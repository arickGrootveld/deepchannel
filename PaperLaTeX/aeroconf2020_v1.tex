...
Utilizing the least squares algorithm, we obtained filter coefficients which transform the measured state matrix (\textbf{Z}) into the best prediction ($\hat{x}$) that we can make given a sequence history of $N$ and a filter order of $M$. 

The Least Squares Filter coefficients $b_{ls}$ can be calculated from:
\begin{equation}
\bm{b_{ls}}=(\bm{Z}^{\rm T}\bm{Z})^{-1}\bm{Z}^{\rm T}\bm{x_{pred}}
\end{equation}
With $\bm{Z}$ and $\bm{x_{pred}}$ defined as
\begin{equation*}
	\bm{x_{pred}} = \left[ \begin{matrix} x[k-1]  \dots x[k-M-1]\end{matrix} \right]^\top\\
\end{equation*}
$\vec{\bm{Z}} = \begin{bmatrix}
   z[k-1]   & ...       & z[k-N-1]     & \\
   z[k-2]   & ...       & z[k-N-2]     & \\
   \vdots             &  \ddots   &              & \\
   z[k-M-1] & ...       & z[k-M-N-1]  
...
where $\bm{x_{pred}}$ is obtained from the System States of the training data, and $\bm{Z}$ is obtained from the Observation States of the training data. 

Once the Least Squares Coefficients are computed, it can be applied with a simple matrix multiplication to get the predicted states:
\begin{equation}
\bm{\hat{x}_{pred}} = \bm{Z} \bm{b_{ls}} \\
\end{equation}



%% ** %% Reformatting this whole section to give a little bit of a better explanation %% ** %%

%The prediction least squares coefficient solutions $a_{ls}$  given by the following equations:
%\begin{eqnarray} 
%{\hat{x}_{pred}} = \bm{Z} \bm{b_{ls}} \\
%\bm{b_{ls}}&=&(\textbf{Z}^{\rm T}\textbf{Z})^{-1}\textbf{Z}^{\rm T}x_{pred}
%\end{eqnarray}
%\begin{align}
%\begin{aligned}
