...
% v1.4, 09/30/2020, AGKlein, Added an initial row of all zeros to
%                            INCLUDE_ZEROS mode, removed "mode" vector and
%                            renamed "mode_all" to "mode", made "SqErr_GKF"
%                            into matrix (pre-averaged) squared-error
%                            to preserve SE data for all realizations,
%                            changed random seed command for Octave-compat.
...
% Training set params --
numSims=5e3;                % number of realizations
...
outfile = 'largeTrain5.mat';      % output filename for storing XX and YY variables for NN
seed = 31423;                   % seed for random generator, for reproducible results

% Validation set params --
%numSims=2e3;               % number of realizations
%N=1e3;                     % length of each realization
...
%seed = 667;                % seed for random generator, for reproducible results
...
%SHUFFLE = false;            % shuffles data (across realizations) before saving
%INCLUDE_ZEROS = false;      % includes initial transient by pre-pending seqLength-1 zeros (useful for showing MSE vs. time on a fully trained NN, generally used with SHUFFLE = false)
%outfile = 'test5.mat';      % output filename for storing XX and YY variables for NN
%seed = 1131;                % seed for random generator, for reproducible results
...
%SHUFFLE = false;            % shuffles data (across realizations) before saving
%INCLUDE_ZEROS = true;       % includes initial transient by pre-pending seqLength-1 zeros (useful for showing MSE vs. time on a fully trained NN, generally used with SHUFFLE = false)
%outfile = 'test6.mat';      % output filename for storing XX and YY variables for NN
%seed = 5684;                   % seed for random generator, for reproducible results
