...
% v1.1, 9/30/2020, AGKlein, Tweaked to acommodate all zeros vector when INCLUDE_ZEROS=true, 
%                           also acommodates per-realization and per-sample squared error data
%                           and generates some additional plots
...
infile1 = 'test6.mat';      % file of input data (target trajectories, system parameters)
infile2 = 'test6_imm.mat';  % file of IMM results
infile3 = 'test6_tcn.mat';  % file of TCN results... should contain a single matrix called YY
                            % with length equal to N*numSims -by- 2
windowSize=20;              % determines how many samples after jump is still 
...
if INCLUDE_ZEROS % for each case, add bias, and prepend with appropriate # of NaNs where TCN didn't estimate first seqLength time steps
    YY=cat(2,NaN*ones(2,1,numSims),YY+cat(2,zeros(2,seqLength,numSims),z_imm(:,2:N-seqLength+1,:)));  
else
    YY=cat(2,NaN*ones(2,seqLength+1,numSims),YY+z_imm(:,2:N+1,:));
end
...
for i=1:numSims
    idxCV2CT=strfind(mode(:,i)',[zeros(windowSize,1); ones(windowSize,1)]')';
    ...
    idxCT2CV=strfind(mode(:,i)',[ones(windowSize,1); zeros(windowSize,1)]')';
...
%% Saving figures
savefig(fig1, "figs/figure1.fig");
savefig(fig2, "figs/figure2.fig");
savefig(fig3, "figs/figure3.fig");
savefig(fig4, "figs/figure4.fig");
savefig(fig5, "figs/figure5.fig");
