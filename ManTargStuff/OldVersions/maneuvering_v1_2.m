...
% Training set params --
%numSims=1e2;                % number of realizations
...
seed = 2;                   % seed for random generator, for reproducible results
...
genie_KF_MSE = squeeze(sampleM_pred(1,1,:)+sampleM_pred(3,3,:));
x_hat_pred_GKF = x_hat_pred; % save last genie KF trajectory for plotting
save(outfile,'XX','YY','z_imm', 'x_imm', 'T', 'P', 'velocity_init_mean', 'velocity_init_var', 'Om_init_mean', 'Om_init_var', 'Om_var', 'Q', 'R', 'SHUFFLE', 'INCLUDE_ZEROS', 'seed', 'genie_KF_MSE', 'x_hat_pred_GKF','mode', 'Om')
...
disp(' ')
disp(['genie KF: ' num2str(mean(genie_KF_MSE(seqLength+2:end)))])
...
figure(3)
plot(0:N2-2,squeeze(M_pred(1,1,2:end)+M_pred(3,3,2:end)),0:N2-2,genie_KF_MSE(2:end))
