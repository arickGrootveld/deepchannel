...
SHUFFLE = false;            % shuffles data (across realizations) before saving
INCLUDE_ZEROS = false;      % includes initial transient by pre-pending seqLength-1 zeros (useful for showing MSE vs. time on a fully trained NN, generally used with SHUFFLE = false)
outfile = 'test3.mat';      % output filename for storing XX and YY variables for NN
seed = 890;                   % seed for random generator, for reproducible results
...
%seed = 3;                   % seed for random generator, for reproducible results
...
x_hat_pred_GKF_allRealizations = zeros(2, size(x_hat_pred, 2), numSims);
predStateInds = [1, 3];
...
for k=1:numSims
    ...
    x_hat_pred_GKF_allRealizations(:, :, k) = x_hat_pred(predStateInds, :);
...
save(outfile,'XX','YY','z_imm', 'x_imm', 'T', 'P', 'velocity_init_mean', 'velocity_init_var', 'position_init_var', 'Om_init_mean', 'Om_init_var', 'Om_var', 'Q', 'R', 'SHUFFLE', 'INCLUDE_ZEROS', 'seed', 'GKF_MSE', 'x_hat_pred_GKF', 'mode', 'Om', 'x_hat_pred_GKF_allRealizations')
