...
infile = 'test1.mat';       % file of input data (target trajectories, system parameters)
outfile = 'test1_imm.mat';  % file where results of IMM get saved
...
M(:,:,1,1) = [diag([0 temp 0 temp]) zeros(4,1); zeros(1,4) 0];  % CV mode init
M(:,:,2,1) = [diag([0 temp 0 temp]) zeros(4,1); zeros(1,4) (Om_init_mean^2+Om_init_var)]; % CT mode init
...
for k=1:numSims
    for n=2:N2
        ...
        %Lambda(Lambda==0)=eps;                         % numerical hack for small Lambdas, doesn't seem necessary
        ...
        x_hat_pred_imm(:,n)=x_hat_pred(:,:,n)*mu(:,n);  % compute combined prediction
...
%% save results
save(outfile,'IMM_MSE','x_hat_pred_imm','mu')
...
Om=x(5);
F = eye(5);
%if abs(Om)>thresh % Bar-Shalom, (11.7.2-4)
if Om^2 % Bar-Shalom, (11.7.2-4)
